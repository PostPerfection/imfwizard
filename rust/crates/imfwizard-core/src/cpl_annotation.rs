...
pub fn read_cpl_annotations(cpl_path: &Path) -> Result<CplVersionInfo, AnnotationError> {
    ...
    let cpl_uuid = extract_tag(&content, "Id")
        ...
        .to_string();
    let title = extract_tag(&content, "ContentTitle").unwrap_or_default();
...
pub fn set_cpl_revision(cpl_path: &Path, revision: &str) -> Result<(), AnnotationError> {
    let ann = CplAnnotation {
        text: format!("Revision set to {revision}"),
        author: "IMF Wizard".to_string(),
