...
fn parse_cmx_edl(file: &Path, fps: f64) -> Result<EdlParseResult, EdlError> {
    ...
    // CMX 3600: EVENT REEL TRACK TRANSITION SRC_IN SRC_OUT REC_IN REC_OUT
    for line in lines {
...
fn parse_cmx_event_line(line: &str, fps: f64) -> Option<EditEvent> {
    ...
    // First field must be a number (event index)
    let index: u32 = parts[0].parse().ok()?;

    // Verify timecode fields (positions 4-7) look like timecodes
...
fn parse_fcp_xml(file: &Path, fps: f64) -> Result<EdlParseResult, EdlError> {
    ...
    // Extract title from first <name> element
    let title = extract_tag_value(&content, "name").unwrap_or_default();

    // Extract clip elements with start/end
    let mut events = Vec::new();
...
mod tests {
    ...
    fn test_parse_cmx_edl() {
        ...
        assert_eq!(result.events[0].src_in, 86400); // 01:00:00:00 at 24fps
        assert_eq!(result.events[0].rec_in, 0);
        assert_eq!(result.events[1].rec_out, 240); // 00:00:10:00 at 24fps
