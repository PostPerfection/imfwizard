...
fn parse_ttml_time(t: &str, fps: f64) -> f64 {
    ...
    if parts.len() == 3 {
        ...
        if let Some((sec, ms)) = parts[2].split_once('.') {
            ...
            // Normalize: "500" means 500ms, "5" means 5ms depending on length
...
pub fn retime_subtitles(opts: &RetimeOptions) -> Result<RetimeResult, RetimeError> {
    ...
    Ok(RetimeResult {
        ...
        entries_processed: entries / 2, // begin+end = 1 subtitle entry
        time_shift_ms: 0.0,
    })
}
...
fn retime_ttml(
    ...
    // Process begin="..." and end="..." attributes
...
/// Find the next begin="..." or end="..." time attribute in the text.
/// Returns (prefix, attr_name, time_value, remaining).
fn find_time_attr(s: &str) -> Option<(&str, &str, &str, &str)> {
    ...
    let (pos, pat) = best?;
    let attr_name = &pat[..pat.len() - 2]; // "begin" or "end"
    let val_start = pos + pat.len();
    let val_end = s[val_start..].find('"')? + val_start;
    let time_val = &s[val_start..val_end];

    // Validate it looks like a time code (starts with digits and colon)
    if time_val.len() >= 8 && time_val.as_bytes()[2] == b':' {
        let prefix = &s[..pos];
        let rest = &s[val_end + 1..]; // after closing quote
        Some((prefix, attr_name, time_val, rest))
    } else {
        // Not a time code, skip past this match and try again
...
fn retime_srt(content: &str, ratio: f64, stretch: bool) -> (String, u32) {
    ...
    while let Some(pos) = find_srt_timecode(remaining) {
        ...
        remaining = rest;
        entries += 2; // count each timecode pair as 2
    }
    output.push_str(remaining);
...
/// Find the next SRT timecode line: "HH:MM:SS,mmm --> HH:MM:SS,mmm"
fn find_srt_timecode(s: &str) -> Option<(&str, &str, &str, &str)> {
    let arrow = " --> ";
    let pos = s.find(arrow)?;

    // Start timecode is 12 chars before arrow: "HH:MM:SS,mmm"
    if pos < 12 {
        // Try further in the string
...
mod tests {
    ...
    fn test_retime_srt_stretch() {
        ...
        // With stretch, times should shift slightly (ratio ~1.001)
        assert!(out_content.contains("-->"));
        // The start time should be slightly earlier than original
...
<tt xmlns="http://www.w3.org/ns/ttml">
  <body>
    ...
    fn test_no_stretch_passthrough() {
        ...
        // With stretch=false, times should remain the same
        let out_content = fs::read_to_string(&output).unwrap();
