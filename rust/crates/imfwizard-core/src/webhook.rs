...
/// Build a JSON payload for a validation result.
pub fn build_validation_payload(imp_dir: &Path, valid: bool, errors: u32, warnings: u32) -> String {
    format!(
        r#"{{"imp_dir":"{}","valid":{},"errors":{},"warnings":{}}}"#,
        escape_json_string(&imp_dir.display().to_string()),
        valid,
        errors,
        warnings
    )
...
pub fn test_webhook(config: &WebhookConfig) -> WebhookResult {
    let event = WebhookEvent {
        ...
        job_id: String::new(),
        payload_json: r#"{"message":"IMF Wizard webhook test"}"#.to_string(),
...
mod tests {
    ...
    fn test_build_event_body_defaults() {
        ...
        // Timestamp should be auto-generated (contains "T" and "Z")
    ...
    fn test_build_job_completed_payload() {
        let payload = build_job_completed_payload("job-1", Path::new("/output/imf"), 125.5);
        assert!(payload.contains(r#""job_id":"job-1""#));
        assert!(payload.contains(r#""status":"completed""#));
        assert!(payload.contains(r#""output_dir":"/output/imf""#));
    ...
    fn test_build_validation_payload() {
        let payload = build_validation_payload(Path::new("/data/imp"), true, 0, 2);
        assert!(payload.contains(r#""imp_dir":"/data/imp""#));
    ...
    fn test_build_validation_payload_invalid() {
        let payload = build_validation_payload(Path::new("/data/imp"), false, 3, 1);
