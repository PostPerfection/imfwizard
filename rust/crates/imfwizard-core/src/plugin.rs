...
pub fn execute_hook(hook: PluginHook, plugins_dir: &Path, context_json: &str) -> bool {
    let Ok(entries) = fs::read_dir(plugins_dir) else {
        return true; // no plugins dir = success
    };
    ...
    for entry in entries.flatten() {
        ...
        let ctx_file = std::env::temp_dir().join("imfwizard_plugin_ctx.json");
...
mod tests {
    ...
    fn test_execute_hook_success() {
        ...
        // Create a simple hook script that exits 0
    ...
    fn test_execute_hook_failure() {
        let tmp = TempDir::new().unwrap();
        let plugin_dir = tmp.path().join("failing_plugin");
        let hooks_dir = plugin_dir.join("hooks");
        fs::create_dir_all(&hooks_dir).unwrap();

        // Create a hook script that exits with error
        fs::write(hooks_dir.join("post_encode.py"), "import sys; sys.exit(1)").unwrap();

        assert!(!execute_hook(PluginHook::PostEncode, tmp.path(), "{}"));
    }
}
